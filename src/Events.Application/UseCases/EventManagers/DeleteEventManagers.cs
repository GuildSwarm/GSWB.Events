...
namespace Events.Application.UseCases.EventManagers
{
    ...
    {
        public async Task<IHttpResult<IEnumerable<EventManagerDetailDTO>>> DeleteManagers(Guid aEventId, string aAccessToken, IEnumerable<MemberKey> aMemberKeyList, CancellationToken aCancellationToken = default)
        {
            ...
            var lAfterDeleteManagersResult = await lEventResult.Bind(anEvent => anEvent.DeleteManagers(aMemberKeyList))
