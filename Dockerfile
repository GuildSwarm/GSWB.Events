...
RUN tar -xvf projectfiles.tar \
    && dotnet restore "src/Events.API/Events.API.csproj" --configfile ./NuGet.docker.config /p:DockerBuild=true -r linux-musl-x64 \
    && rm projectfiles.tar  # Remove the tar file to reduce image size
COPY . .
RUN dotnet build "src/Events.API/Events.API.csproj" -c $BUILD_CONFIGURATION --no-restore /p:DockerBuild=true -r linux-musl-x64

FROM build AS publish
ARG BUILD_CONFIGURATION
RUN dotnet publish "src/Events.API/Events.API.csproj" -c $BUILD_CONFIGURATION --no-build -o /app/publish /p:UseAppHost=true /p:DockerBuild=true -r linux-musl-x64

FROM registry.guildswarm.org/baseimages/dotnet_base AS final 
WORKDIR /app
COPY --from=publish /app/publish .
COPY Infrastructure/EventsEntrypointOverride.sh ./entrypoint.sh
